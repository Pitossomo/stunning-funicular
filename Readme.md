...
- Inicialmente, o teste irá falhar caso não haja a validação, portanto devemos implementá-la:
    ...
    ```
- Por fim, incluiremos o teste para o caso de ocorrer no UserController algum erro, que será simulado isolando a variável `let mockReturnCreateUser: jest.Mock<any, any>`
- Nosso arquivo de testes *UserController.test.ts* contemplará 3 testes, um para cada situação:
    ```typescript
    import { makeMockRequest } from '../__mocks__/mockRequest'
    import { makeMockResponse } from '../__mocks__/mockResponse'
    import { UserController } from './UserController'
